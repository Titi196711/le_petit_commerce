...
let FooPicker = (function () {
  'use strict';

  let hasEventListener = window.addEventListener;
  let weeks = ['L', 'M', 'M', 'J', 'V', 'S', 'D'];
  ...
  function FooPicker() {
    ...
    _self.showPicker = function() {
      ...
      let pickerDiv = document.getElementById('foopicker-' + _id);
      if (pickerField) {
    ...
    _self.removeListeners = function(id) {
      ...
      if (picker) {
        let el = picker.getElementsByClassName('foopicker__day');
        if (el && el.length) {
          for (let count = 0; count < el.length; count++) {
            if (typeof el[count].onclick === "function") {
              let elem = document.getElementById(id + '-foopicker__day--' + (count + 1));
    ...
    _self.updateCalendar = function(newMonth, newYear) {
      ...
      let pickerDiv = document.getElementById('foopicker-' + _id);
      if (pickerDiv) {
        let datepicker = pickerDiv.querySelector('.foopicker');
    ...
    _self.handleDocumentClick = function(event) {
      ...
      let pickerDiv = document.getElementById('foopicker-' + _self.options.id);
      _self.isPickerClicked = false;
      _self.monthChange = false;
  ...
  let Calendar = {
    ...
    buildMonths: function() {
      let elem = '<select class="foopicker__date--month">', month = this.month();
    ...
    // Build years select
    buildYears: function() {
      let elem = '<select class="foopicker__date--year">', currentYear = this.getCurrentYear();
      let year = this.year();
    ...
    addListeners: function(instance) {
      ...
      let pickerDiv = document.getElementById('foopicker-' + id);
      if (pickerDiv) {
        let prevBtn = pickerDiv.getElementsByClassName('foopicker__arrow--prev')[0];
        let nextBtn = pickerDiv.getElementsByClassName('foopicker__arrow--next')[0];
        addEvent(prevBtn, 'click', instance.changeMonth, false);
        addEvent(nextBtn, 'click', instance.changeMonth, false);

        let monthSelect = pickerDiv.getElementsByClassName('foopicker__date--month')[0];
        let yearSelect = pickerDiv.getElementsByClassName('foopicker__date--year')[0];
      ...
      let el = pickerDiv.getElementsByClassName('foopicker__day');
      if (el && el.length) {
        for (let count = 0; count < el.length; count++) {
          if (typeof el[count].onclick !== "function") {
    ...
    modifyDateClass: function(instance) {
      let id = instance.options.id, day = instance.selectedDay, currentDate, disabled;
      let date = new Date();
      let month = date.getMonth(), year = date.getFullYear(), dayClass;
      let pickerDiv = document.getElementById('foopicker-' + id);
      if (pickerDiv) {
        let el = pickerDiv.getElementsByClassName('foopicker__day');
        if (el && el.length) {
